...
const checkRelationship = (elderIdParam = 'elderId') => {
  return async (req, res, next) => {
    ...
    // Doctor must verify a doctor_patient_links assignment (cross-service call - health-service
    // never holds the users_db pool, and doctors' assigned patients are not embedded in the JWT)
    if (role === 'DOCTOR') {
      try {
